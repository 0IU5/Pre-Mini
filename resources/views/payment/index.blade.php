        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">ID Paket</th>
            ...
            <tbody>
                @foreach ($payment as $p)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4">{{ $p->metode_pembayaran }}</td>
                        <td class="px-6 py-4">
                            <img src="{{ asset('storage/' . $p->bukti_transaksi) }}" alt="Bukti Transaksi" class="w-20">
                        </td>
                        <td class="px-6 py-4">{{ $p->id }}</td>
                        <td class="px-6 py-4">{{ $p->id_paket }}</td>
                        <td class="px-6 py-4">
                            <a href="{{ route('payment.edit', $p->id) }}" class="text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                            <form action="{{ route('payment.destroy', $p->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
